...
public class WorldManager : MonoBehaviour
{
    ...
    void GenerateWorld()
    {
        ...
        while (targetWorldSize > worldSize)
        {
            ...
            Vector3 currentTileSpawnPos = nextTileSpawnPos + new Vector3(((prefabTilesList[random].tileLength - 1) * 10) / 2, 0, 0);
            GameObject tile = Instantiate(prefabTilesList[random].prefabTile, currentTileSpawnPos, Quaternion.identity);
    ...
    void MoveTile(bool _rightDirection) // false = left direction
    {
        if (_rightDirection)
        {
            ...
            movedTile.tile.transform.position = new Vector3(endPosition.x + 10 + (((movedTile.length - 1) * 10) / 2), 0, 0);
